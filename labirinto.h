...
public:
    ...
    //calcula heuristica
    void calc_heuristica(const Coord& dest);

    bool operator<( NoH& old);
    bool operator == (const NoH& compara)const;
    bool operator == (const Coord& compara)const;
    bool operator != (const Coord& dest);
    Coord operator + (const Coord& p);
    friend class maior_que;
...
public:
    ...
    maior_que(double h);
    bool operator() (const NoH& elem_n) const;
    friend class Labirinto;
